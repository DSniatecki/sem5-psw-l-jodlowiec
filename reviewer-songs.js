...
function showWebsiteSettings(shouldShowOpacity) {
    document.getElementById("websiteSettingsSelectors").innerHTML = `
        <p><label>Reviews theme color: <input id="reviewsColor" type="color" value="${websiteSettings.reviewsColor}"></label></p>
        <p><label>Reviews font size: <input id="reviewsFontSize" type="range" min="12" max="28" value="${websiteSettings.reviewsFontSize}"></label>
            <output id="reviewsFontSizeValue">${websiteSettings.reviewsFontSize}px</output></p>
    `
    document.getElementById("reviewsColor").addEventListener("change", event => {
        websiteSettings.reviewsColor = event.target.value
        loadReviews()
    })
    document.getElementById("reviewsFontSize").addEventListener("change", event => {
        const newFontSize = event.target.value;
        websiteSettings.reviewsFontSize = newFontSize
        document.getElementById("reviewsFontSizeValue").innerText = `${newFontSize}px`
        loadReviews()
    })
    if (shouldShowOpacity) {
        document.getElementById("websiteSettingsSelectors").innerHTML += ` 
        <p>
        <label>Reviews opacity: <input id="reviewsOpacity" type="range" min="80" max="100" value="${websiteSettings.reviewsOpacity}"> <output id="reviewsOpacityValue">${websiteSettings.reviewsOpacity}%</output>
            </label>
        </p>
        `
        document.getElementById("reviewsOpacity").addEventListener("change", event => {
            const newOpacity = event.target.value;
            websiteSettings.reviewsOpacity = newOpacity
            document.getElementById("reviewsOpacityValue").innerText = `${newOpacity}%`
            loadReviews()
        })
    }
}

function manageWebsiteSettings(event) {
    if (websiteSettings.isSettingsSelectorOpen) {
        if (event.altKey) {
            setTimeout(() => {
                document.getElementById("websiteSettingsSelectors").innerHTML = ""
                websiteSettings.isSettingsSelectorOpen = false;
            }, 2000);
        } else {
            document.getElementById("websiteSettingsSelectors").innerHTML = "";
            websiteSettings.isSettingsSelectorOpen = false;
        ...
        showWebsiteSettings(event.shiftKey)
        websiteSettings.isSettingsSelectorOpen = true;
    }
}

function loadWebsite() {
    document.getElementById("cursorPosition").addEventListener("mousemove", e => {
        document.getElementById("cursorPosition").innerHTML =
            `<p>
                    <strong>Screen X/Y:</strong> ${e.screenX}, ${e.screenY}
                    <strong>Client X/Y:</strong> ${e.clientX}, ${e.clientY}
             </p>
            `;
    })
    document.getElementById("cursorPosition").addEventListener("mouseleave", e => {
        document.getElementById("cursorPosition").innerHTML = ``
    })

    document.getElementById("addNewReviewForm").addEventListener("submit", addNewReviewFromFromEvent)
    document.getElementById("websiteSettingsSection").addEventListener("click", manageWebsiteSettings)
    loadReviews();
}
